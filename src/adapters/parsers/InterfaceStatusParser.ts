...
export class InterfaceStatusParser {
  static parse(output: string): Partial<Interface>[] {
    ...
    const standardRegex = /^(?<port>\S+)\s+(?<desc>.*?)\s+(?<status>connected|notconnect|disabled|err-disabled|inactive)\s+/;
    // Regex for Cisco SMB "show interface description"
    const smbRegex = /^(?<port>gi\d+|fa\d+)\s+(?:Up|Down)\s+\S+\s+\S+\s+\S+\s+(?<desc>.*)$/;
