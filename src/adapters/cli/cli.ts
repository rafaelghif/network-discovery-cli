...
import { cliOptionsSchema, CliOptions } from "./schemas.js";
...
export async function getInteractiveOptions(): Promise<CliOptions> {
  ...
  if (validationResult.success) {
    ...
    validationResult.error.errors.forEach((err) =>
      console.error(`Validation Error: ${err.path.join(".")} - ${err.message}`)
    );
    process.exit(1);
  }
}
...
export function getNonInteractiveOptions(): CliOptions {
  try {
    ...
    if (!configJson)
      throw new Error(
        "NDC_CONFIG_JSON env var not set for non-interactive mode."
      );
